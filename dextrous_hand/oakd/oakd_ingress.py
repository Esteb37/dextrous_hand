...
from dextrous_hand.oakd.oakd_utils import PointCloudVisualizer, calibrate_with_aruco, compute_projection_matrix
        ...
        with device:
            ...
            while True:
                ...
                if key == ord("s"):
                    ...
                    cv2.imwrite(f"{serial_no}_{timestamp}_color.png", color)  # type: ignore
                    if has_depth:
                        cv2.imwrite(f"{serial_no}_{timestamp}_depth.png", depth_vis)  # type: ignore
                        cv2.imwrite(
