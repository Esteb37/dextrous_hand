...
def LOG_INFO(log):
    logging.get_logger('DynamixelClient').info(log)

def LOG_ERROR(log):
    logging.get_logger('DynamixelClient').error(log)

def LOG_WARN(log):
    logging.get_logger('DynamixelClient').warn(log)
...
def dynamixel_cleanup_handler():
    ...
    for open_client in open_clients:
        if open_client.port_handler.is_using:
            LOG_WARN('Forcing client to close.')
